drop table if exists likes;
drop table if exists users_excursion;
drop table if exists comment;
drop table if exists users;
drop table if exists excursion;

-- валидация значений на уровне БД не позволит внести
-- некорректные значения вручную
-- валидация на уровне DTO и сущностей
-- позволит настроить
-- оповещение пользователя об ошибке

create table users
(
    id            int generated by default as identity primary key,
    name          varchar(100)            not null,
    email         varchar(100) unique     not null,
    phone_number  varchar(10) unique      not null,
    password      varchar(256)            not null,
    registered_at timestamp default now() not null,
    role          varchar(100)            not null
);
create unique index users_unique_email_idx ON users (email);

create table excursion
(
    id          int generated by default as identity primary key,
    name        varchar(100) not null,
    date        date,
    description varchar(500),
    price       int check ( price >= 0 )
);

create table users_excursion
(
    users_id     int references users (id) on delete cascade,
    excursion_id int references excursion (id) on delete cascade,
    primary key (users_id, excursion_id)
);

create table comment
(
    id           int generated by default as identity primary key,
    message      varchar(300)            not null,
    created_at   timestamp default now() not null,
    users_id     int                     references users (id) on delete set null,
    excursion_id int                     references excursion on delete set null
);
create unique index comment_unique_excursion_created_at_idx on comment (excursion_id, created_at);

create table likes
(
    id           int generated by default as identity primary key,
    users_id     int references users (id) on delete cascade,
    excursion_id int references excursion (id) on delete cascade,
    constraint like_unique unique (users_id, excursion_id)
);

